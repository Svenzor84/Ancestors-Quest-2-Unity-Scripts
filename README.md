...
Scripts are all written in C# using the Unity API.  If you are here to play test, or to look at some code, I appreciate your interest!  Below is a link to a drive space where you will find Windows, Linux, Mac, and Android versions of AQ2.  Feel free to play and distribute, and please submit any issues, comments, complaints, or suggestions using the Issues function of this site (click on the Issues tab above, and then the green New Issue button to alert the team of anything that might need their attention).  
Read below for Release Notes and Have fun!
...
-exp cost is now tied to the exp spent varaible instead of the current value of the stat to be increased
[each stat increase costs 100 exp + 100 * the current value of expSpent variable, which is indicated on screen by player beard length]


BUGS/GLITCHES FOUND IN V1.2 (FIXED IN V1.2.1)
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

-when enemies take damage that drops their health to negative, health bar grows in the negative direction
[health bar growth now only happens at hp > 0, otherwise the percent variable for health bar length is set to 0.0]

-on floors greater than 3 the secret room spawns no kindly man and no exit, player can move around outside of the map, but cannot exit the room
[secret floor tile choice is now based on random number instead of floor number, floor tile array cannot be outpaced]

-skip button on main menu scene only works at specific times (before any animation transitions occur)
[added skip transitions and animations to all required menu scene objects so that skip button works at all stages of the scene]

-player was able to abuse double click spells by continuously clicking
[added a playerClick bool that trips when a spell is cast and resets after the player's turn has ended]

-enemy adds were able to attack the player after the boss was dead, resulting in death
[boss death now triggers the roomOver bool, which prevents enemies from attacking as the player leaves a room]

-Beholder upper left/right sections do not transition to death sequence if casting a spell
[added transition from casting animation to death animation]

-armor misaligned, shows lower layer sprite beneath
[player sprite material was set to a custom material with pixel snap enabled, switching back to default sprite material fixed the issue]

-player can move and attack as a floating equipment pile and blood pool after being killed
[added a check for the roomOver bool in the player movement logic and set roomOver to true on player death]
...
-constitution health gain was computed using the player's old maxHP
[maxHP is now recalculated before con healing is done, giving the player max possible benefit]

-rooms could become too big (at high floor level) to display properly on screen
[max room size was reduced from 20 columns to 15 and tested on lowest available resolution]

-UI does not scale down with resolution, and aspect ratio's other than 16:9 can hide UI elements
[limited available aspect ratios to 16:9; quick and temporary fix]

-exit tile could spawn directly next to the player upon entering a new room
[limited random exit tile position to the two sides opposite the player's starting position]

-enemy could attack the player after having moved onto the exit tile, during room transition
[created a new roomOver bool that stops enemy turns (just like skipTurn bool)]
